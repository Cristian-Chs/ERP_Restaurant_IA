...
def get_exchange_rate():
    """Obtiene la tasa de VES del backend"""
    try:
        r = requests.get(CURRENCY_RATES_URL, timeout=3)
        if r.status_code == 200:
            return float(r.json().get("VES", 1.0))
    except Exception as e:
        logging.error(f"Error obteniendo tasa: {e}")
    return 35.0 # Fallback razonable
...
async def handle_state_idle(update: Update, context: ContextTypes.DEFAULT_TYPE, texto: str):
    ...
    if not productos:
        # 0. Manejo explícito de respuestas negativas ("no", "nada más")
...
async def handle_state_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "confirm_order":
        ...
        context.user_data[pedido_key] = {
            ...
            "precio": price,
            "precio_original": price,  # Guardar precio original para cálculos
        ...
        # Build keyboard with discount options
        keyboard = []
...
async def handle_apply_points(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Aplica puntos de fidelidad como descuento al pedido
    """
    query = update.callback_query
    await query.answer()
    ...
    # Parse callback data: "apply_points:50:pedido_carrito"
    ...
    # Update price
    precio_original = data.get("precio_original", data["precio"])
    nuevo_precio = max(0, precio_original - discount)
    
    data["precio"] = nuevo_precio
    data["descuento_aplicado"] = discount
    data["puntos_usados"] = points_to_use
    context.user_data[pedido_key] = data
...
async def handle_enter_coupon(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Solicita al usuario que ingrese un código de cupón
    """
    query = update.callback_query
    await query.answer()
    ...
    # Parse callback data: "enter_coupon:pedido_carrito"
    pedido_key = query.data.split(":")[1]
    
    # Save state to wait for coupon code
    context.user_data["waiting_coupon_code"] = True
...
async def handle_coupon_validation(update: Update, context: ContextTypes.DEFAULT_TYPE, coupon_code: str):
    ...
    pedido_key = context.user_data.get("coupon_pedido_key", "pedido_carrito")
    data = context.user_data.get(pedido_key)
    ...
    # Search for coupon
    from bot.models import Coupon, RedeemedCoupon
    
    coupon = await sync_to_async(
        lambda: Coupon.objects.filter(code=coupon_code.upper()).first()
    )()
    ...
    already_used = await sync_to_async(
        lambda: RedeemedCoupon.objects.filter(
            telegram_id=telegram_id,
            coupon=coupon
        ).exists()
    )()
    ...
    # Calculate discount
    precio_original = data.get("precio_original", data["precio"])
    discount = await sync_to_async(coupon.apply_discount)(precio_original)
    ...
    data["precio"] = nuevo_precio
    data["descuento_aplicado"] = discount
    data["cupon_usado"] = coupon_code.upper()
    data["cupon_id"] = coupon.id
    context.user_data[pedido_key] = data
    
    # Clear coupon state
    context.user_data["waiting_coupon_code"] = False
    context.user_data.pop("coupon_pedido_key", None)
    
    # Update coupon usage
    def increment_usage():
        coupon.current_uses += 1
        coupon.save()
    await sync_to_async(increment_usage)()
...
async def show_payment_info(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query:
        ...
        # If sending new message as fallback
        final_markup = reply_markup if reply_markup else ReplyKeyboardRemove()
        await update.message.reply_text(text=mensaje_pago, parse_mode="Markdown", reply_markup=final_markup)
...
async def handle_payment_approval(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        try:
            ...
            order_obj = await sync_to_async(Order.objects.get)(id=order_id)
            
            generator = InvoiceGenerator()
            # Run blocking image generation in executor
            invoice_path = await sync_to_async(generator.generate)(order_obj)
...
def run_dijkstra_ai(lat, lon):
    """Calcula ruta usando Dijkstra real sobre OSM"""
    try:
        finder = PathFinder()
        # Ensure map is loaded (this is blocked, maybe load on startup in apps.py is better for prod)
        # But for this bot script, we load it here or check.
        map_path = os.path.join(os.path.dirname(__file__), "map.osm")
        finder.load_map(map_path)
        
        # Coordenadas del restaurante (Fijos según el código frontend anterior: 11.411, -69.673)
        restaurant_loc = (11.670464, -70.151655) 
        user_loc = (float(lat), float(lon))
        
        result = finder.find_path(restaurant_loc, user_loc)
        return result
    except Exception as e:
        print(f"Error en Dijkstra: {e}")
        return {
            "distance": "Calc Failed",
            "time": "? min",
            "path": []
        }
...
async def handle_location(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Run Pathfinding
    route_info = run_dijkstra_ai(lat, lon)
    
    distancia = route_info.get("distance", "N/A")
    ...
    # Continue flow...
    # (Checking if we need to call logic from handle_message 0.2)
    # Actually, location usually comes via the 0.2 flow in handle_message if text,
    # or here if attachment.
    # User might send location attachment instead of text address.
    
    # Set address for logic
    context.user_data["waiting_address_late"] = False # Reset flag if it was set
    address_str = f"GPS: {lat:.5f}, {lon:.5f}"
    
    # ... Resume logic similar to handle_message 0.2 ...
    order_id = context.user_data.get("current_order_id")
    # If we are in the flow of verifying payment + location:
    if order_id:
         # Notify admin
         await notify_admin_new_order(
            context, 
